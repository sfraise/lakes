...
class PIGEN {
	...
	public function pigen_activate() { // Check the server whether or not it has imageMagick enabled.
		if ( function_exists('exec') ) {
			exec( 'convert -version', $arr, $res ); //Try to get ImageMagick "convert" program version number.
			if ( $res === 0 && count($arr) > 2 ) {
				exec( 'which gs', $arr, $res ); 
				if ( $res === 0 ){
	...
	public function pigen_options() { 
		...
		if ( isset($_GET['run']) ) {
			...
			echo
				...
				"\t\t".'</div>'."\n".
				"\t\t".'<div>'."\n".
				"\t\t\t".'<p><span class="float"><input type="radio" id="pigen_verify_imageMagick" name="pigen_verify_imagick" value="imageMagick" '.( isset($opt['verify_imagick']) && $opt['verify_imagick'] === 'imageMagick' ? 'checked="checked"' : $imagemagick ).' /><label for="pigen_verify_imageMagick">'.__( 'Use imageMagick with exec function', 'pdf-image-generator' ).'</label></span><span class="float"><input type="radio" id="pigen_verify_imagick" name="pigen_verify_imagick" value="imagick" '.( isset($opt['verify_imagick']) && $opt['verify_imagick'] === 'imagick' ? 'checked="checked"' : $imagick ).' /><label for="pigen_verify_imagick">'.__( 'Use imagick PHP Extension', 'pdf-image-generator' ).'</label></span></p>'."\n".
				"\t\t".'</div>'."\n".
				"\t".'</fieldset>'."\n".
	...
	public function pigen_generate( $file ){ // Generate thumbnail from PDF
		...
		if ( $verify_imagick == 'imagick' ) {
			...
			if ( $property ) {
				...
				$imageMagick = "convert -density 150 -set units PixelsPerInch -density 72 -quality {$quality} -background {$image_bgcolor} -flatten -resize {$max_width}x{$max_height} {$file}[0] {$file_url}";
			} else {
	...
	public function pigen_attachment( $attachment_id ){ // Generate thumbnail from PDF
		if( get_post_mime_type( $attachment_id ) === 'application/pdf' ){
			...
			// GET ADOBE META DATA
			$company = trim(html_entity_decode(strip_tags(getXmpData($file, 'dc:title'))));
			$zipcode = html_entity_decode(strip_tags(getXmpData($file, 'dc:description')));
			$categories = html_entity_decode(strip_tags(getXmpData($file, 'dc:creator')));

			// ADD CUSTOM FIELD VALUES
			if($company) {
				update_post_meta($attachment_id, '_pigen_company_name', $company);
			}
			if($zipcode) {
				update_post_meta($attachment_id, '_pigen_zip_code', $zipcode);
			}
			if($categories) {
				update_post_meta($attachment_id, '_pigen_categories', $categories);
			}
		}
...
<?php }
	...
	public function pigen_attachment_fields_to_edit( $form_fields, $post ) {
		...
		$form_fields['pigen-company-name'] = array(
			'label' => 'Company Name',
			'input' => 'text',
			'value' => get_post_meta( $post->ID, '_pigen_company_name', true )
		);

		$form_fields['pigen-zip-code'] = array(
			'label' => 'Zip-Code',
			'input' => 'text',
			'value' => get_post_meta( $post->ID, '_pigen_zip_code', true )
		);

		$form_fields['pigen-categories'] = array(
			'label' => 'Categories',
			'input' => 'text',
			'value' => get_post_meta( $post->ID, '_pigen_categories', true )
		);
	...
	public function pigen_attachment_fields_to_save( $post, $attachment ){
		...
		if( isset( $attachment['pigen-company-name'] ) )
			update_post_meta( $post['ID'], '_pigen_company_name', $attachment['pigen-company-name'] );
		if( isset( $attachment['pigen-zip-code'] ) )
			update_post_meta( $post['ID'], '_pigen_zip_code', $attachment['pigen-zip-code'] );
		if( isset( $attachment['pigen-categories'] ) )
			update_post_meta( $post['ID'], '_pigen_categories', $attachment['pigen-categories'] );
