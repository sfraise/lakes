...
/*** db example
$querystr = "
SELECT " . $wpdb->prefix . "geo_usa.*
FROM " . $wpdb->prefix . "geo_usa
WHERE " . $wpdb->prefix . "geo_usa.id = 1
ORDER BY " . $wpdb->prefix . "geo_usa.id DESC
";

$geoResults = $wpdb->get_results($querystr, OBJECT);

foreach($geoResults as $geoResult) {
echo $geoResult->country_name;
}
 */
...
class Zipcode {
    ...
    function get_zip_details($zip) {
        ...
        $querystr = "
            SELECT " . $wpdb->prefix . "geo_usa.latitude AS latitude, " . $wpdb->prefix . "geo_usa.longitude AS longitude, " . $wpdb->prefix . "geo_usa.name, " . $wpdb->prefix . "geo_usa.adm_name1, " . $wpdb->prefix . "geo_usa.adm_code1
            FROM " . $wpdb->prefix . "geo_usa
            WHERE " . $wpdb->prefix . "geo_usa.postal_code = '$zip'
         ";
    ...
    function get_zip_point($zip) {
        ...
        $querystr = "
            SELECT " . $wpdb->prefix . "geo_usa.latitude, " . $wpdb->prefix . "geo_usa.longitude
            FROM " . $wpdb->prefix . "geo_usa
            WHERE " . $wpdb->prefix . "geo_usa.postal_code = '$zip'
         ";
        ...
        if($geoResults) {
            ...
            foreach($geoResults as $geoResult) {
                $result[0] = $geoResult->latitude;
                $result[1] = $geoResult->longitude;
            }
    ...
    function get_zips_in_range($zip, $range, $sort=1, $include_base) {
        ...
        $querystr = "
            ...
            FROM " . $wpdb->prefix . "geo_usa
            WHERE " . $wpdb->prefix . "geo_usa.postal_code != '$zip'
            AND " . $wpdb->prefix . "geo_usa.latitude BETWEEN '$min_lat' AND '$max_lat'
            AND " . $wpdb->prefix . "geo_usa.longitude BETWEEN '$min_lon' AND '$max_lon'
         ";
        ...
        if($geoResults) {
            foreach($geoResults as $geoResult) {
                ...
                $dist = $this->calculate_mileage($details[0],$geoResult->latitude,$details[1],$geoResult->longitude);
                if($this->units == _UNIT_KILOMETERS) {
                    $dist = $dist * _M2KM_FACTOR;
                }
                if($dist <= $range) {
                    $return[str_pad($geoResult->postal_code, 5, "0", STR_PAD_LEFT)] = round($dist, $this->decimals);
                }
            }
        }
